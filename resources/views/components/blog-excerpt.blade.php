...
@foreach($blogs as $blog)
    ...
    @if($loop->iteration == 1 || $loop->iteration == 5)
        <div class="col-6 mt-3x fixed-height"><a href="{{$blog->categories->slug}}/{{$blog->slug}}"><img src="{{asset($blog->image_path)}}" class="w-100"></a></div>
        <div class="col-6 mt-3x @if($loop->iteration == 1) big-blue-user @else big-bluew big-bluew-gap @endif fixed-height">
            ...
            <a href="{{$blog->categories->slug}}/{{$blog->slug}}">
            <h2 class="mt-3 deepTittle
                    @if($loop->iteration == 5)
                    black
                    @endif
                    ">{{$blog->title}}</h2></a>
            <a href="{{$blog->categories->slug}}/{{$blog->slug}}"><span class="whiteParagraph">{!! $blog->excerpt!!}</span></a>
    ...
    @else
        <div class="col-3x mt-3"><a href="{{$blog->categories->slug}}/{{$blog->slug}}"><img class="imageq" src="{{asset($blog->image_thumbnail)}}"></a>
            ...
            <a href="{{$blog->categories->slug}}/{{$blog->slug}}"><h2 class="card-title">{{$blog->title}}</h2></a>
            <a href="{{$blog->categories->slug}}/{{$blog->slug}}"><span class="paragraphCard">{!! $blog->excerpt !!}</span></a>
